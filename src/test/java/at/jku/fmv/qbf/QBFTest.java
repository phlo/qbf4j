...
public class QBFTest {
	...
	void test_illegalConstruction() {
		...
		List<QBF> subformulasSingle = Arrays.asList(var);
		...
		assertThrows(IllegalArgumentException.class, () -> new ForAll(var, variablesNull));
		...
		assertThrows(IllegalArgumentException.class, () -> new Exists(var, variablesNull));
	...
	@DisplayName("equals")
	void test_equals() {
		QBF var = new Variable(x1);
		QBF and = new And(new Variable(x1), new Variable(x2), new Variable(x3));
		QBF or = new Or(new Variable(x1), new Variable(x2), new Variable(x3));
		QBF not = new Not(or);
		QBF forall = new ForAll(not, x2);
		QBF exists = new Exists(forall, x1);

		// Variable
		assertEquals(var, new Variable(x1));
		assertNotEquals(var, new Variable(x2));
		assertNotEquals(var, and);

		// Not
		assertEquals(not, new Not(new Or(new Variable(x1), new Variable(x2), new Variable(x3))));
		assertNotEquals(not, new Not(var));
		assertNotEquals(not, and);

		// And
		assertEquals(and, new And(new Variable(x1), new Variable(x2), new Variable(x3)));
		assertNotEquals(and, new And(new Variable(x1), new Variable(x2)));
		assertNotEquals(and, or);

		// Or
		assertEquals(or, new Or(new Variable(x1), new Variable(x2), new Variable(x3)));
		assertNotEquals(or, new Or(new Variable(x1), new Variable(x2)));
		assertNotEquals(or, and);

		// ForAll
		assertEquals(forall, new ForAll(new Not(new Or(new Variable(x1), new Variable(x2), new Variable(x3))), x2));
		...
		assertEquals(
			...
			new Exists(
				new ForAll(
					new Not(new Or(new Variable(x1), new Variable(x2), new Variable(x3))),
					x2),
				x1));
	...
	@DisplayName("stream")
	void test_stream() {
		QBF var1 = new Variable(x1);
		QBF var2 = new Variable(x2);
		QBF var3 = new Variable(x3);
		QBF and = new And(new Variable(x1), new Variable(x2), new Variable(x3), QBF.True);
		QBF or = new Or(new Variable(x1), new Variable(x2), new Variable(x3), QBF.False);
		QBF not = new Not(or);
		QBF forall = new ForAll(not, x2);
		QBF exists = new Exists(forall, x1);
		...
		// pre-order
		assertEquals(
			var1.stream(Traverse.PreOrder).collect(Collectors.toList()),
			Arrays.asList(var1));
		assertEquals(
			and.stream(Traverse.PreOrder).collect(Collectors.toList()),
			Arrays.asList(and, var1, var2, var3, QBF.True));
		assertEquals(
			or.stream(Traverse.PreOrder).collect(Collectors.toList()),
			Arrays.asList(or, var1, var2, var3, QBF.False));
		assertEquals(
			not.stream(Traverse.PreOrder).collect(Collectors.toList()),
			Arrays.asList(not, or, var1, var2, var3, QBF.False));
		assertEquals(
			forall.stream(Traverse.PreOrder).collect(Collectors.toList()),
			Arrays.asList(forall, not, or, var1, var2, var3, QBF.False));
		assertEquals(
			exists.stream(Traverse.PreOrder).collect(Collectors.toList()),
			Arrays.asList(exists, forall, not, or, var1, var2, var3, QBF.False));

		// post-order
		assertEquals(
			var1.stream(Traverse.PostOrder).collect(Collectors.toList()),
			Arrays.asList(var1));
		assertEquals(
			and.stream(Traverse.PostOrder).collect(Collectors.toList()),
			Arrays.asList(var1, var2, var3, QBF.True, and));
		assertEquals(
			or.stream(Traverse.PostOrder).collect(Collectors.toList()),
			Arrays.asList(var1, var2, var3, QBF.False, or));
		assertEquals(
			not.stream(Traverse.PostOrder).collect(Collectors.toList()),
			Arrays.asList(var1, var2, var3, QBF.False, or, not));
		assertEquals(
			forall.stream(Traverse.PostOrder).collect(Collectors.toList()),
			Arrays.asList(var1, var2, var3, QBF.False, or, not, forall));
		assertEquals(
			exists.stream(Traverse.PostOrder).collect(Collectors.toList()),
			Arrays.asList(var1, var2, var3, QBF.False, or, not, forall, exists));
	}
	...
	void test_streamVariables() {
		...
		QBF var2 = new Variable(x2);
		QBF var3 = new Variable(x3);
		QBF var4 = new Variable(x4);
		QBF and = new And(var1, var2, var3);
		QBF or = new Or(and, var4);
		QBF not = new Not(or);
		QBF forall = new ForAll(not, x2);
		QBF exists = new Exists(forall, x1);
		...
		assertEquals(
			"x1",
			joinCommaDelimited(var1.streamVariables()));
	...
	void test_streamFreeVariables() {
		...
		assertEquals(
			"x1",
			joinCommaDelimited(var1.streamFreeVariables()));
	...
	void test_streamBoundVariables() {
		...
		assertEquals(
			"",
			joinCommaDelimited(var1.streamBoundVariables()));
	...
	void test_getCriticalPaths() {
		...
		QBF samePaths =
			new And(
				new ForAll(new Exists(new Variable("x3"), "x2"), "x1"),
				new ForAll(new Exists(new Variable("x6"), "x5"), "x4"),
				new ForAll(new Exists(new Variable("x9"), "x8"), "x7"));
	...
	void test_getSkeleton() {
		...
		QBF qbf = new ForAll(new Exists(new Variable(x3), x2), x1);
	...
	void test_unifyPrefix() {
		QBF var1 = new Variable(x1);
		QBF var2 = new Variable(x2);
		QBF not = new Not(var1);
		QBF and = new And(not, var2);
		QBF or = new Or(var1, and);
		...
		assertEquals(var1, var1.unifyPrefix());
	...
	void test_negate() {
		QBF var = new Variable(x1);
		QBF not = new Not(var);
		QBF and = new And(new Variable(x1), new Variable(x2));
		QBF or = new Or(new Not(new Variable(x1)), var);
		...
		assertEquals(var.negate(), not);
		assertEquals(not.negate(), var);
	...
	@DisplayName("toNNF")
	void test_toNNF() {
		QBF var = new Not(new Variable(x1));
		QBF and = new Not(new And(new Variable(x1), new Variable(x2), QBF.True));
		QBF or = new Not(new Or(var, new Not(new Variable(x2)), QBF.False));
		...
		assertEquals(forall.toNNF(), new Exists(new And(new Variable(x1), new Variable(x2), QBF.True), x1, x2));
		assertEquals(
			exists.toNNF(),
			new ForAll(
				new ForAll(
					new Or(new Not(new Variable(x1)), new Not(new Variable(x2)), QBF.False),
					x1, x2),
				x3));
	}
	...
	void test_toString() {
		QBF and = new And(new Variable(x1), new Variable(x2), new Variable (x3));
		QBF or = new Or(new Not(new Variable(x1)), and);
